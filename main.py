...
def def_main():
    while True:
        print("*" * 31 + "BRINGIT.ME" + "*" * 31)                                            #Repeat Menu until stops.
...
def def_order_menu():                                                                               #yousef
    while True: 
        print("*" * 31 + "BRINGIT.ME" + "*" * 31)                                            # While looping to keep menu alive
...
def def_full_file_reader():                                                                         #mustafa
    ...
    while i <= (len(list_foods) - 1): #Enumarte through food list to filter out prices and setup print Formatting by replacing spaces with count difference of string length and align Prices to the most left of the terminal
        if 'USD' in list_foods[i]:
            list_foods[i] = str(list_foods[i][:list_foods[i].index('USD') - 1]) + ' ' * (20 - (list_foods[i].index('USD') - 1)) + str(list_foods[i][list_foods[i].index('USD'):])
        i += 1

    i = 0
    while i <= (len(list_drinks) - 1):
        if 'USD' in list_drinks[i]:
            list_drinks[i] = str(list_drinks[i][:list_drinks[i].index('USD') - 1]) + ' ' * (20 - (list_drinks[i].index('USD') - 1)) + str(list_drinks[i][list_drinks[i].index('USD'):])
        i += 1

    i = 0
    while i <= (len(list_services) - 1):
        if 'USD' in list_services[i]:
            list_services[i] = str(list_services[i][:list_services[i].index('USD') - 1]) + ' ' * (20 - (list_services[i].index('USD') - 1)) + str(list_services[i][list_services[i].index('USD'):])
        i += 1
...
def def_file_sorter(): # Applying Sorting to the array to be sorted from A-Z ASC ((AND)) Extracting out prices after sorting and appending them to a prices array accordingly to a parrallel indexes
    ...
    while i < len(list_foods):
        list_item_price[i] = float(list_foods[i][int(list_foods[i].index("USD") + 3):]) # Extracting Out "RM" + [SPACE] from and cast out the string into an integer
        i += 1

    i = 0
    while i < len(list_drinks):
        list_item_price[40 + i] = float(list_drinks[i][int(list_drinks[i].index("USD") + 3):]) # Applying extraction on 40 and above items which are the drinks
        i += 1

    i = 0
    while i < len(list_services):
        list_item_price[80 + i] = float(list_services[i][int(list_services[i].index("USD") + 3):]) # Applying extraction on 80 and above items wich are Services
        i += 1
def_file_sorter()

def def_food_drink_order():
    while True:
            print("*" * 31 + "BRINGIT.ME" + "*" * 31)
    ...
    while True:
        ...
        if total_price > var_discount_3: ### price > 5000
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                "" + " " * 17 + "DISCOUNT AMOUNTS:   USD " + str(round(total_price * var_discount_3_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
                "" + " " * 17 + "TOTAL PRICES:       USD " + str(round(total_price, 2)) + "\n" + " " * 17 + "*" * 35 # Round() to flour the float into an interger
            print(" " * 17 + "-" * 35 + "\n"
                "" + " " * 17 + "DISCOUNT RATES:      % " + str(var_discount_3_rate * 100) + "\n"
                "" + " " * 17 + "DISCOUNT AMOUNTS:   USD " + str(round(total_price * var_discount_3_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n"
                "" + " " * 17 + "TOTAL PRICES:       USD " + str(round(total_price, 2)))
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                "" + " " * 17 + "DISCOUNT AMOUNTS:   USD " + str(round(total_price * var_discount_2_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
                "" + " " * 17 + "TOTAL PRICES:       USD " + str(round(total_price, 2)) + "\n" + " " * 17 + "*" * 35 # Round() to flour the float into an interger
            print(" " * 17 + "-" * 35 + "\n"
                "" + " " * 17 + "DISCOUNT RATES:      % " + str(var_discount_2_rate * 100) + "\n"
                "" + " " * 17 + "DISCOUNT AMOUNTS:   USD " + str(round(total_price * var_discount_2_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n"
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                "" + " " * 17 + "DISCOUNT AMOUNTS:   USD " + str(round(total_price * var_discount_1_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
                "" + " " * 17 + "TOTAL PRICES:       USD " + str(round(total_price, 2)) + "\n" + " " * 17 + "*" * 35 # Round() to flour the float into an interger
            print(" " * 17 + "-" * 35 + "\n"
                "" + " " * 17 + "DISCOUNT RATES:      % " + str(var_discount_1_rate * 100) + "\n"
                "" + " " * 17 + "DISCOUNT AMOUNTS:   USD " + str(round(total_price * var_discount_1_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n"
